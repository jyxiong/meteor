...
// --------------------------------------------------------------------
template <class T> inline CUDABuffer<T>::CUDABuffer() {}

// --------------------------------------------------------------------
template <class T> inline void CUDABuffer<T>::allocate(size_t size) {
  if (isAllocated())
    free();
  m_size = size;
...
template <class T> inline void CUDABuffer<T>::free() {
  if (isAllocated())
    cuda_free(d_ptr);
  d_ptr = 0;
  m_size = 0;
...
inline void CUDABuffer<T>::copyToDevice(const T *data, size_t size) {
  ...
  CUDA_CHECK(cudaMemcpy(reinterpret_cast<void *>(d_ptr), data, size,
                        cudaMemcpyHostToDevice));
}

// --------------------------------------------------------------------
template <class T>
inline void CUDABuffer<T>::copyToDeviceAsync(const std::vector<T> &vec,
                                             const CUstream &stream) {
  copyToDeviceAsync(vec.data(), sizeof(T) * vec.size(), stream);
}

template <class T>
inline void CUDABuffer<T>::copyToDeviceAsync(const T *data, size_t size,
                                             const CUstream &stream) {
  if (!isAllocated())
    allocate(size);

  CUDA_CHECK(cudaMemcpyAsync(reinterpret_cast<void *>(d_ptr), data, size,
                             cudaMemcpyHostToDevice, stream));
}

// --------------------------------------------------------------------
template <class T> inline T *CUDABuffer<T>::copyFromDevice() {
  T *h_ptr = static_cast<T *>(malloc(m_size));
  if (!isAllocated()) {
    // pgLogFatal("The device-side data hasn't been allocated yet.");
    return nullptr;
  }
  CUDA_CHECK(cudaMemcpy(h_ptr, reinterpret_cast<T *>(d_ptr), m_size,
...
// --------------------------------------------------------------------
template <class T> inline bool CUDABuffer<T>::isAllocated() const {
  return static_cast<bool>(d_ptr);
}

template <class T> inline CUdeviceptr CUDABuffer<T>::devicePtr() const {
  return d_ptr;
}

template <class T> inline T *CUDABuffer<T>::deviceData() {
  return reinterpret_cast<T *>(d_ptr);
}

template <class T> inline size_t CUDABuffer<T>::size() const { return m_size; }
