...
Context::Context(unsigned int device_id) : m_device_id(device_id) {
  m_options = {
      &contextLogCallback,                     // logCallbackFunction
      nullptr,                                 // logCallbackData
      4,                                       // logCallbackLevel
...
void Context::create() {
  ...
  ASSERT((int32_t)m_device_id < num_device,
         "The device ID exceeds the detected number of GPU devices.");
...
void Context::destroy() {
  if (m_ctx)
    OPTIX_CHECK(optixDeviceContextDestroy(m_ctx));
  m_ctx = nullptr;
}
...
OptixDeviceContextOptions Context::options() const { return m_options; }

// --------------------------------------------------------------------
void Context::setDeviceId(const unsigned int device_id) {
  m_device_id = device_id;
}
unsigned int Context::deviceId() const { return m_device_id; }
