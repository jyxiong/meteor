...
// GeometryAccel -------------------------------------------------------------
GeometryAccel::GeometryAccel(ShapeType shape_type) : m_shape_type(shape_type) {
...
void GeometryAccel::build(const Context &ctx, CUstream stream) {
  ...
  for (size_t i = 0; i < m_shapes.size(); i++) {
    ...
    if (!all_type_equal) {
      ...
      THROW("All build input types of shapes must be same as type of "
            "GeometryAccel.");
    }
  ...
  OPTIX_CHECK(optixAccelComputeMemoryUsage(
      static_cast<OptixDeviceContext>(ctx), &m_options, m_build_inputs.data(),
      static_cast<uint32_t>(m_build_inputs.size()), &gas_buffer_sizes));
  ...
  OPTIX_CHECK(optixAccelBuild(
      static_cast<OptixDeviceContext>(ctx), stream, &m_options,
      m_build_inputs.data(), m_build_inputs.size(), d_temp_buffer,
      gas_buffer_sizes.tempSizeInBytes,
      d_buffer_temp_output_gas_and_compacted_size,
      gas_buffer_sizes.outputSizeInBytes, &m_handle,
      num_emit_properties > 0 ? &emit_property : nullptr, num_emit_properties));
  ...
  if ((m_options.buildFlags & OPTIX_BUILD_FLAG_ALLOW_COMPACTION) != 0) {
    ...
    if (compacted_gas_size < gas_buffer_sizes.outputSizeInBytes) {
      CUDA_CHECK(
          cudaMalloc(reinterpret_cast<void **>(&d_buffer), compacted_gas_size));
      OPTIX_CHECK(optixAccelCompact(static_cast<OptixDeviceContext>(ctx), 0,
                                    m_handle, d_buffer, compacted_gas_size,
                                    &m_handle));
...
void GeometryAccel::update(const Context &ctx, CUstream stream) {
  ...
  OPTIX_CHECK(optixAccelBuild(
      static_cast<OptixDeviceContext>(ctx), stream, &m_options,
      m_build_inputs.data(), m_build_inputs.size(), d_temp_buffer,
      d_temp_buffer_size, d_buffer, d_buffer_size, &m_handle, nullptr, 0));
...
// ---------------------------------------------------------------------------
uint32_t GeometryAccel::count() const { return m_count; }

OptixTraversableHandle GeometryAccel::handle() const { return m_handle; }

CUdeviceptr GeometryAccel::deviceBuffer() const { return d_buffer; }

size_t GeometryAccel::deviceBufferSize() const { return d_buffer_size; }

bool GeometryAccel::isBuilded() const { return (bool)d_buffer; }
