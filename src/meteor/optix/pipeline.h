...
public:
  ...
  explicit Pipeline(const OptixPipelineCompileOptions &c_op,
                    const OptixPipelineLinkOptions &l_op);
  ...
  [[nodiscard]] Module
  createBuiltinIntersectionModule(const Context &ctx,
                                  OptixPrimitiveType primitive_type);

  [[nodiscard]] Module createModuleFromPtxSource(const Context &ctx,
                                                 const std::string &source);

  [[nodiscard]] ProgramGroup createRaygenProgram(const Context &ctx,
                                                 const Module &module,
                                                 const std::string &func_name);
  [[nodiscard]] ProgramGroup createRaygenProgram(const Context &ctx,
                                                 const ProgramEntry &entry);
  [[nodiscard]] ProgramGroup createMissProgram(const Context &ctx,
                                               const Module &module,
                                               const std::string &func_name);
  [[nodiscard]] ProgramGroup createMissProgram(const Context &ctx,
                                               const ProgramEntry &entry);
  [[nodiscard]] ProgramGroup createHitgroupProgram(const Context &ctx,
                                                   const Module &module,
                                                   const std::string &ch_name);
  [[nodiscard]] ProgramGroup
  createHitgroupProgram(const Context &ctx, const ProgramEntry &ch_entry);
  [[nodiscard]] ProgramGroup createHitgroupProgram(const Context &ctx,
                                                   const Module &module,
                                                   const std::string &ch_name,
  ...
  [[nodiscard]] std::pair<ProgramGroup, uint32_t>
  createCallablesProgram(const Context &ctx, const Module &module,
                         const std::string &dc_name,
                         const std::string &cc_name);
  [[nodiscard]] std::pair<ProgramGroup, uint32_t>
  createCallablesProgram(const Context &ctx, const ProgramEntry &dc_entry,
                         const ProgramEntry &cc_entry);
  [[nodiscard]] ProgramGroup
  createExceptionProgram(const Context &ctx, const Module &module,
                         const std::string &func_name);
  [[nodiscard]] ProgramGroup createExceptionProgram(const Context &ctx,
                                                    const ProgramEntry &entry);

  /** Create pipeline object and calculate the stack sizes of pipeline. */
  void create(const Context &ctx);
  void createFromPrograms(const Context &ctx,
                          const std::vector<ProgramGroup> &prgs);
  void destroy();
