...
constexpr SBTRecordType operator|(SBTRecordType t1, SBTRecordType t2) {
  return static_cast<SBTRecordType>((uint32_t)t1 | (uint32_t)t2);
}
constexpr SBTRecordType operator|(uint32_t t1, SBTRecordType t2) {
  return static_cast<SBTRecordType>(t1 | (uint32_t)t2);
}
constexpr SBTRecordType operator&(SBTRecordType t1, SBTRecordType t2) {
  return static_cast<SBTRecordType>((uint32_t)t1 & (uint32_t)t2);
}
...
constexpr SBTRecordType operator~(SBTRecordType t1) {
  return static_cast<SBTRecordType>(~(uint32_t)t1);
}
constexpr uint32_t operator+(SBTRecordType t1) {
  return static_cast<uint32_t>(t1);
}

/* Default structs for shader binding table records. */
template <class Cam> struct pgRaygenData { typename Cam::Data camera; };
...
template <typename RaygenRecord, typename MissRecord, typename HitgroupRecord,
          typename CallablesRecord, typename ExceptionRecord, uint32_t N>
...
public:
  ...
  RaygenRecord &raygenRecord() { return m_raygen_record; }
  const RaygenRecord &raygenRecord() const { return m_raygen_record; }
  ...
  MissRecord &missRecord(const int idx) { return m_miss_records[idx]; }
  ...
  /* Hitgroup */
  void
  addHitgroupRecord(const std::array<HitgroupRecord, N> &hitgroup_records) {
  ...
  void updateHitgroupRecordOnDevice() {
    ...
    m_sbt.hitgroupRecordCount =
        static_cast<uint32_t>(m_hitgroup_records.size());
  ...
  void addCallablesRecord(const CallablesRecordArgs &...args) {
    static_assert(
        std::conjunction<
            std::is_same<CallablesRecord, CallablesRecordArgs>...>::value,
  ...
  void deleteCallablesRecord(const int idx) { return; }
  ...
  ExceptionRecord &exceptionRecord() { return m_exception_record; }
  const ExceptionRecord &exceptionRecord() const { return m_exception_record; }
  ...
  void createOnDevice() {
    ...
    d_exception_record.copyToDevice(&m_exception_record,
    ...
    m_sbt.callablesRecordCount =
        static_cast<uint32_t>(m_callables_records.size());
    m_sbt.callablesRecordStrideInBytes =
        static_cast<uint32_t>(sizeof(CallablesRecord));
  ...
  OptixShaderBindingTable &sbt() { return m_sbt; }

  bool isOnDevice() { return on_device; }

private:
  template <class Head, class... Args>
  static void pushToVector(std::vector<Head> &v, const Head &head,
                           const Args &...args) {
    v.emplace_back(head);
...
template <class Cam> using pgRaygenRecord = Record<pgRaygenData<Cam>>;
...
using emptyRecord = Record<pgEmptyData>;

template <class Cam, uint32_t N>
using pgDefaultSBT =
    ShaderBindingTable<pgRaygenRecord<Cam>, pgMissRecord, pgHitgroupRecord,
                       pgCallablesRecord, pgExceptionRecord, N>;
